...
public class PistonEvent implements Listener {
    ...
    private void addPistonData(PistonTemplate pistonTemplate, int blockX, int blockY, int blockZ) {
        for (GrimPlayer player : GrimAPI.INSTANCE.getPlayerDataManager().getEntries()) {
            if (!player.compensatedWorld.isChunkLoaded(blockX >> 4, blockZ >> 4) || !isCloseEnough(blockX, blockY, blockZ, player.compensatedEntities.self.trackedServerPosition.getPos().getX(), player.compensatedEntities.self.trackedServerPosition.getPos().getY(), player.compensatedEntities.self.trackedServerPosition.getPos().getZ())) continue;
